...
typedef tuple<i32, i32, i64, i64> edge;
...
private:
  ...
  bool bfs(i32 s, i32 t) { // find augmenting path
    ...
    while (!q.empty()) {
      ...
      for (auto &idx : g[u]) {                         // explore neighbors of u
        auto &[v, _u, cap, flow] = edges[idx];         // stored in EL[idx]
  ...
  i64 send_one_flow(i32 s, i32 t, i64 f = inf) { // send one flow from s->t
    ...
    auto &[u, idx] = p[t];
    auto &cap = get<2>(edges[idx]), &flow = get<3>(edges[idx]);

    i64 pushed = send_one_flow(s, u, min(f, cap - flow));
    flow += pushed;

    auto &rflow = get<3>(edges[idx ^ 1]); // back edge
  ...
  i64 dfs(i32 u, i32 t, i64 f = inf) { // traverse from s->t
    ...
    for (i32 &i = last[u]; i < (i32)g[u].size(); ++i) { // from last edge
      auto &[v, _u, cap, flow] = edges[g[u][i]];
      ...
      if (i64 pushed = dfs(v, t, min(f, cap - flow))) {
        ...
        auto &rflow = get<3>(edges[g[u][i] ^ 1]); // back edge
        rflow -= pushed;
...
public:
  ...
  void add_edge(i32 u, i32 v, i64 w, bool directed = true) {
    ...
    edges.emplace_back(v, u, w, 0);        // u->v, cap w, flow 0
    g[u].push_back((i32)edges.size() - 1); // remember this index

    edges.emplace_back(u, v, directed ? 0 : w, 0); // back edge
  ...
  auto &get_edges() { return this->edges; }

  auto debug_edges() {
    for (auto &[v, u, cap, flow] : edges) {
      if (flow > 0 && u != 0 && v != V - 1) {
        debug(u / 2 + u % 2, "->", v / 2 + v % 2, flow);
      }
    }
  }
};
